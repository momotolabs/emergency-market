...
import { Fieldset, Input, Select, Map, Button, ErrorValidation, Label } from "@/Client/Components/Form";
...
const props = defineProps(['me', 'states', 'cities', 'contract_url']);
...
const inputs = useForm({
    ...
    first_time: true,
    google_link: props.me.company.social?.google_link,
    ...
    website_link: props.me.company.social?.website_link,
    youtube_link: props.me.company.social?.youtube_link,
    ...
    signature: props.me.provider_profile.signature,
});
...
const rules = computed(() => ({
    ...
    google_link: {  url },
    kind: { required },
    miles: { required, integer, maxValue: maxValue(100) },
    name: { required },
...
const submit = async () => {
  ...
  await inputs.put(route('dashboard.profile.update'), {
    onError(errors){
...
<template>
  <AdminLayout>
    <ProfileTabNav />
    <div class="md:max-w-[70%] mx-auto mb-5">
      <Fieldset legend="Contract Link">
        <Input label="Public contract link" v-model="props.contract_url" disabled />
        <span class="hint">Copy and paste this link on your website</span>
      </Fieldset>
      <form @submit.prevent="submit" class="block">
        <Fieldset legend="Generals" class="space-y-5">
          ...
          >
            ...
            <!-- <option value="water_mitigation" disabled>Water Mitigation</option>
            <option value="tarping_roofing_and_board_up" disabled>Tarping roofing and board up</option> -->
          </Select>
          <Input
        ...
        <Fieldset legend="Parkin lot generals" class="space-y-5">
          ...
          <div>
            ...
            <img :src="props.me.provider_profile.signature" />
            <VueSignaturePad
                  class="border-2"
                  width="100%"
                  height="200px"
                  ref="signaturePad"
                />
            <button @click="clearSignature" type="button" class="bg-gray-300 font-bold py-2 w-full">Clear Signature</button>
        ...
        <div class="flex space-x-4 mt-5">
          <Button type="submit" :loading="inputs.processing">Update profile</Button>
        </div>
      </form>
    </div>
  </AdminLayout>
